...
(def specs #{["/status/:status-id" :get (conj json-interceptors `routes.status/get-status) :route-name :get-status]
             ...
             ["/workflow/:workflow-id/transition/status-from" :get (conj json-interceptors `routes.transition/get-transition-by-status-from) :route-name :get-all-initial-transitions]
             ["/workflow/:workflow-id/transition/status-from/:status-from" :get (conj json-interceptors `routes.transition/get-transition-by-status-from) :route-name :get-transition-by-status-from]})
